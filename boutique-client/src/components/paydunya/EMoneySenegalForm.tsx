...
export default function EMoneySenegalForm({ 
  paymentToken,
  customerName,
  customerEmail,
  customerPhone, 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/process-e-money-senegal-payment', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        alert('Paiement E-Money Senegal réussi ! ' + data.message)
        onSuccess(data)
      } else {
        const errorMessage = data.message || 'Erreur lors du paiement E-Money Senegal'
        alert('Erreur de paiement E-Money Senegal: ' + errorMessage)
        onError(errorMessage)
      }
    } catch (error: any) {
      const errorMessage = error.message || 'Erreur de communication avec E-Money Senegal'
      alert('Erreur de paiement E-Money Senegal: ' + errorMessage)
      onError(errorMessage)
    } finally {
      setIsLoading(false)
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex items-center mb-6">
        ...
        />
        <div>
          <h3 className="text-lg font-semibold text-gray-900">E-Money Senegal</h3>
          <p className="text-sm text-gray-600">Paiement mobile via E-Money</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-start">
            ...
            <div className="text-sm text-blue-800">
              ...
              <ul className="list-disc list-inside space-y-1 text-xs">
                <li>Assurez-vous d'avoir un compte E-Money actif</li>
                <li>Vous recevrez un SMS de confirmation</li>
                <li>Entrez votre code PIN E-Money pour confirmer</li>
                <li>Le montant sera débité de votre compte E-Money</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        >
          {isLoading ? (
            ...
            `Payer ${formatAmount(amount, currency)} avec E-Money`
          )}
        </button>
      </form>
    </div>

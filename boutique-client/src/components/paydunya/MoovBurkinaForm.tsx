...
interface MoovBurkinaFormProps {
  paymentToken: string;
  customerName: string;
...
const MoovBurkinaForm: React.FC<MoovBurkinaFormProps> = ({
  paymentToken,
  customerName,
  customerEmail,
  amount,
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const laravelApiUrl = `${process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000'}/api/process-moov-burkina-payment`;
      ...
      if (response.data.success) {
        ...
        setUssdCode(response.data.ussd_code || '*555*6#');
        onSuccess?.(response.data);
      } else {
        setStatus('error');
        const errorMessage = response.data.message || 'Une erreur est survenue lors du paiement Moov Burkina Faso.';
    ...
    return (
      <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-6">
        <div className="flex items-center mb-4">
          ...
          <div className="ml-3">
            <h3 className="text-sm font-medium text-green-800">Paiement Moov Burkina Faso Initié !</h3>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-blue-700">
                <p className="font-semibold">Code USSD à composer :</p>
                <p className="text-lg font-mono bg-blue-100 px-2 py-1 rounded mt-1">{ussdCode}</p>
                <p className="mt-2">1. Composez ce code sur votre téléphone</p>
                <p>2. Suivez les instructions vocales</p>
                <p>3. Confirmez le paiement</p>
                <p>4. Vous recevrez une confirmation par SMS</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mb-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-blue-800">
                Instructions Moov Burkina Faso
              </h3>
              <div className="mt-2 text-sm text-blue-700 space-y-1">
                ...
                <p>4. Après soumission, vous recevrez un code USSD</p>
                <p>5. Composez le code USSD pour finaliser le paiement</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Format : xxxxxxxxx (numéro Moov Burkina Faso)
          </p>
        </div>
...
export default MoovBurkinaForm; 
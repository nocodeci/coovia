...
interface AirtelMoneyZambiaFormProps {
  paymentToken: string
  customerName: string
...
export default function AirtelMoneyZambiaForm({ 
  paymentToken,
  customerName,
  customerEmail,
  customerPhone, 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/pawapay/process/ZMB/airtel-money-zambia', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        alert('Paiement Airtel Money Zambia réussi ! ' + data.message)
        onSuccess(data)
      } else {
        const errorMessage = data.message || 'Erreur lors du paiement Airtel Money Zambia'
        alert('Erreur de paiement Airtel Money Zambia: ' + errorMessage)
        onError({ message: errorMessage })
      }
    } catch (error: any) {
      const errorMessage = error.message || 'Erreur de communication avec Airtel Money Zambia'
      alert('Erreur de paiement Airtel Money Zambia: ' + errorMessage)
              onError({ message: errorMessage })
    } finally {
      setIsLoading(false)
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex items-center mb-6">
        <img 
          src="https://assets.cdn.moneroo.io/icons/circle/airtel_money.svg" 
          alt="Airtel Money Zambia" 
          className="w-8 h-8 mr-3"
        />
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Airtel Money Zambia</h3>
          <p className="text-sm text-gray-600">Paiement mobile via Airtel Money</p>
        </div>
      </div>
      ...
      <div className="mb-6 p-4 bg-blue-50 rounded-lg">
        <div className="flex justify-between items-center">
          ...
          <span className="text-lg font-bold text-blue-600">{formatAmount(amount, currency)}</span>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-start">
            ...
            <div className="text-sm text-blue-800">
              ...
              <ul className="list-disc list-inside space-y-1 text-xs">
                <li>Assurez-vous d'avoir un compte Airtel Money actif</li>
                <li>Vous recevrez un SMS de confirmation</li>
                <li>Entrez votre code PIN Airtel Money pour confirmer</li>
                <li>Le montant sera débité de votre compte Airtel Money</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        >
          {isLoading ? (
            ...
            `Payer ${formatAmount(amount, currency)} avec Airtel Money`
          )}
        </button>
      </form>
    </div>

...
interface SettingsNavModernProps {
  children: React.ReactNode
  className?: string
}

export function SettingsNavModern({ children, className }: SettingsNavModernProps) {
  return (
    <div 
      className={cn(
        "relative bg-background hidden w-full min-w-40 px-1 py-2 md:block",
        "rounded-xl border border-border/50 shadow-xl hover:shadow-2xl transition-all duration-500",
        ...
        "after:absolute after:inset-0 after:rounded-xl after:bg-gradient-to-t after:from-background/30 after:to-transparent after:pointer-events-none",
        className
      )}
      style={{
        position: "relative",

...
interface OrderListTableProps {
  orderData?: any[]
  onDynamicIslandTrigger?: (view: "search" | "filter" | "actions" | "analytics" | "notifications") => void
}

export function OrderListTable({ orderData, onDynamicIslandTrigger }: OrderListTableProps) {
  ...
  const handleAction = async (action: string, order: any) => {
    console.log(`${action} pour la commande:`, order?.order_number || "global")
    ...
    switch (action) {
      ...
      case "archive":
        try {
          // Simulation de l'archivage car updateOrder n'existe pas encore
          setData(prev => 
            prev.map(o => 
              o.id === order.id 
                ? { ...o, status: 'archived' }
      ...
      case "export":
        try {
          ...
          // Ici vous pouvez ajouter la logique d'export réelle
          setTimeout(() => {
        ...
        <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button 
              ...
              size="sm"
              onClick={() => {
                onDynamicIslandTrigger?.("actions")
                // Déclencher l'action d'export après un délai pour laisser le Dynamic Island s'afficher
                setTimeout(() => {
                  // Simuler un clic sur le bouton Export du Dynamic Island
            ...
            <Button 
              variant="outline" 
              size="sm"
              onClick={() => onDynamicIslandTrigger?.("actions")}
            >
